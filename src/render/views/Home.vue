...
const connectSocket = async () => {
  if (serverUrl.value) {
    ...
    store.socket.on('close_reason', (reason: keyof typeof DisconnectReasons) => {
      if (reason in DisconnectReasons) toast.error(DisconnectMessages[reason], toastOptions);
    });
...
<template>
  <div class="w-screen h-screen flex flex-col items-center justify-center text-center">
    ...
    <ConnectHandlerVue v-if="isShowingConnectHandler"
    ...
    <div v-if="isShowingAudioControls" :class="[showAudioControls ? 'fade-enter-active' : 'fade-leave-active']"
      class="mt-10">
